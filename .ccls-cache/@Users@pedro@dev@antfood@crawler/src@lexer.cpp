...
std::vector<std::string> Parser::split()
{
  ...
  while(1)
  {
  size_t pos = m_filename.find(m_delimiter, m_cursor);

  if(pos == std::string::npos)
  {
      size_t dot_pos = m_filename.find(".", m_cursor);
      output.push_back(m_filename.substr(m_cursor, dot_pos - m_cursor ));
      m_cursor = dot_pos;
      output.push_back(m_filename.substr(m_cursor, m_length));
      break;
  };

  output.push_back(m_filename.substr(m_cursor, pos - m_cursor));
